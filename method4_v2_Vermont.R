...
T_A = function(u,sigma) (applyFilter(x = u, kernel = convKernel(sigma = sigma, k = 'gaussian')))     
...
piece_estimate <-  function(t_start, t_end, sigma_range,theta_prior0,N2,pt_range,dt_range,beta0,It){
    ...
    for(n_s in 1:length(sigma_range)){
        ...
        for(i in t_start:t_end){         
            ...
            prob = res$prob
                        pior_theta_A = T_A(post_theta,sigma)
                        ProbA = 10^2*prob*pior_theta_A            post_theta = ProbA
...
cal_I_sample <-  function(pt_range,dt_range,beta0,It,i,N2){
    ...
    for(n_p in 1:length(pt_range)){
        for(n_d in 1:length(dt_range)){
            ...
            d = dt_range[n_d]
            beta_t = beta0*p
            beta_t[seq(1,N2,1)> d] = 0             beta_t_samples[n_sample,] = beta_t
            I_pred = 0
            for(tao in 1:(i-1)){
                I_pred = I_pred + beta_t[i-tao]*It[tao]            }
            I_pred_samples[n_p,n_d] = I_pred
            prob[n_p,n_d] = dpois(round(It[i]),I_pred)        }
    }
